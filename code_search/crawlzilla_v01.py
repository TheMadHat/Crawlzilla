...
async def add_urls_to_queue(queue_pool, urls):
    """Adds a batch of URLs to the database queue with deduplication."""
    async with queue_pool.acquire() as conn:
        async with conn.transaction():
            await conn.executemany(
                ...
                ON CONFLICT (url) DO NOTHING;
                """,
                [(url, url_priority(url), 0) for url in urls]
            )
...
async def main():
    ...
    try:
        ...
        print(f"Database connection pool established")
        queue_pool = await asyncpg.create_pool(queue_dsn)
        logging.info("Queue database connection pool established")
        print(f"Queue database connection pool established")
