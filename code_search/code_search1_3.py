            ...
            if caas_body_div:
                ...
                for sentence in sentences:
                    if search_string.lower() in sentence.lower():
                        ...
                        links_in_sentence = [link for link in caas_body_div.find_all('a', href=True) if sentence in link.parent.get_text()]
                ...
                for _, anchor_text, link_destination in temp_link_data_batch:
                    logging.info(f"Link found - Anchor text: '{anchor_text}', Destination: '{link_destination}'")
                ...
                if len(article_data_batch) >= batch_size:
                    await insert_data(pool, article_data_batch, temp_link_data_batch, link_data_batch)
...
async def insert_data(pool, article_data_batch, temp_link_data_batch, link_data_batch):
    async with pool.acquire() as conn:
        async with conn.transaction():
                ...
                if result:
                    ...
                    for i, link_data in enumerate(temp_link_data_batch):
                        if link_data[0] is None:
                            temp_link_data_batch[i] = (article_id, link_data[1], link_data[2])

            if temp_link_data_batch:
                logging.info(f"Inserting links: {temp_link_data_batch}")
                await conn.executemany("""
                INSERT INTO links (article_id, anchor_text, link_destination)
                VALUES ($1, $2, $3)
                ON CONFLICT (article_id, anchor_text, link_destination) DO NOTHING
                """, temp_link_data_batch)
                link_data_batch.extend(temp_link_data_batch)
...
async def crawl_urls(start_url, search_string, pool, request_timeout, url_limit, include_list, concurrent_tasks, batch_size, crawled_urls):
    ...
    total_queued_urls = 1  # Start with 1 since the start_url is already in the queue
                ...
                if url not in crawled_urls:
                    ...
                    for link in new_links:
                        ...
                        if normalized_link not in crawled_urls:
                            ...
                            total_queued_urls += 1  # Increment the count of queued URLs
