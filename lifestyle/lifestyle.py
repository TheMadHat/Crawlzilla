...
async def get_queries_in_chunks(pool, chunk_size=1000):
    """Retrieves queries in chunks from the 'query' table."""
    async with pool.acquire() as conn:
        async with conn.transaction():
            offset = 0
            while True:
...
async def process_url(pool, url, queries_chunk, session, semaphore):
    """Processes a single URL with a chunk of queries."""
    global total_errors
    global total_warnings  # Add this line if total_warnings is used in this function
    ...
    async with semaphore:
        try:
            ...
            for query_data in queries_chunk:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            with Progress(
                ...
                total_urls = await pool.fetchval("""SELECT COUNT(*) FROM one_p""")
                task = progress.add_task("Processing", total=total_urls, completed=0)
                async for queries in get_queries_in_chunks(pool, chunk_size):
